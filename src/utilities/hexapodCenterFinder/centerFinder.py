...
class HexapodCenterFinder:
    def __init__(
        ...
        if step_size <= 0 or max_travel <= 0:
            raise ValueError("Step size and maximum travel must be greater than zero.")
        if samples < 1:
    ...
    def _step(self, axis, direction):
        target = self.offset[axis] + direction * self.step_size
